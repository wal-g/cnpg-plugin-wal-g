...
func DeleteAllBackupsAndWALsInStorage(
	...
	if err != nil {
		logger.Error(err, "Error occured while deleting all backups: cannot fetch backups list")
	} else {
		...
		for i := len(backupsList) - 1; i >= 0; i-- {
			if backupsList[i].IsPermanent {
				...
				if err != nil {
					logger.Error(err, "Error occured while deleting all backups: cannot unmark permanent backup")
				}
			}
		}
	}
