//go:build !ignore_autogenerated

/*
Copyright 2025 YANDEX LLC.

Licensed under the Apache License, Version 2.0 (the "License");
you may not use this file except in compliance with the License.
You may obtain a copy of the License at

    http://www.apache.org/licenses/LICENSE-2.0

Unless required by applicable law or agreed to in writing, software
distributed under the License is distributed on an "AS IS" BASIS,
WITHOUT WARRANTIES OR CONDITIONS OF ANY KIND, either express or implied.
See the License for the specific language governing permissions and
limitations under the License.
*/

// Code generated by controller-gen. DO NOT EDIT.

package v1beta1

import (
	"k8s.io/api/core/v1"
	metav1 "k8s.io/apimachinery/pkg/apis/meta/v1"
	runtime "k8s.io/apimachinery/pkg/runtime"
)

// DeepCopyInto is an autogenerated deepcopy function, copying the receiver, writing into out. in must be non-nil.
func (in *BackupConfig) DeepCopyInto(out *BackupConfig) {
	*out = *in
	out.TypeMeta = in.TypeMeta
	in.ObjectMeta.DeepCopyInto(&out.ObjectMeta)
	in.Spec.DeepCopyInto(&out.Spec)
	in.Status.DeepCopyInto(&out.Status)
}

// DeepCopy is an autogenerated deepcopy function, copying the receiver, creating a new BackupConfig.
func (in *BackupConfig) DeepCopy() *BackupConfig {
	if in == nil {
		return nil
	}
	out := new(BackupConfig)
	in.DeepCopyInto(out)
	return out
}

// DeepCopyObject is an autogenerated deepcopy function, copying the receiver, creating a new runtime.Object.
func (in *BackupConfig) DeepCopyObject() runtime.Object {
	if c := in.DeepCopy(); c != nil {
		return c
	}
	return nil
}

// DeepCopyInto is an autogenerated deepcopy function, copying the receiver, writing into out. in must be non-nil.
func (in *BackupConfigList) DeepCopyInto(out *BackupConfigList) {
	*out = *in
	out.TypeMeta = in.TypeMeta
	in.ListMeta.DeepCopyInto(&out.ListMeta)
	if in.Items != nil {
		in, out := &in.Items, &out.Items
		*out = make([]BackupConfig, len(*in))
		for i := range *in {
			(*in)[i].DeepCopyInto(&(*out)[i])
		}
	}
}

// DeepCopy is an autogenerated deepcopy function, copying the receiver, creating a new BackupConfigList.
func (in *BackupConfigList) DeepCopy() *BackupConfigList {
	if in == nil {
		return nil
	}
	out := new(BackupConfigList)
	in.DeepCopyInto(out)
	return out
}

// DeepCopyObject is an autogenerated deepcopy function, copying the receiver, creating a new runtime.Object.
func (in *BackupConfigList) DeepCopyObject() runtime.Object {
	if c := in.DeepCopy(); c != nil {
		return c
	}
	return nil
}

// DeepCopyInto is an autogenerated deepcopy function, copying the receiver, writing into out. in must be non-nil.
func (in *BackupConfigSpec) DeepCopyInto(out *BackupConfigSpec) {
	*out = *in
	if in.Walg != nil {
		in, out := &in.Walg, &out.Walg
		*out = new(WalgConfig)
		(*in).DeepCopyInto(*out)
	}
	if in.DownloadConcurrency != nil {
		in, out := &in.DownloadConcurrency, &out.DownloadConcurrency
		*out = new(int)
		**out = **in
	}
	if in.UploadDiskRateLimit != nil {
		in, out := &in.UploadDiskRateLimit, &out.UploadDiskRateLimit
		*out = new(int)
		**out = **in
	}
	if in.UploadNetworkRateLimit != nil {
		in, out := &in.UploadNetworkRateLimit, &out.UploadNetworkRateLimit
		*out = new(int)
		**out = **in
	}
	if in.UploadConcurrency != nil {
		in, out := &in.UploadConcurrency, &out.UploadConcurrency
		*out = new(int)
		**out = **in
	}
	if in.UploadDiskConcurrency != nil {
		in, out := &in.UploadDiskConcurrency, &out.UploadDiskConcurrency
		*out = new(int)
		**out = **in
	}
	if in.TarSizeThreshold != nil {
		in, out := &in.TarSizeThreshold, &out.TarSizeThreshold
		*out = new(int64)
		**out = **in
	}
	out.Retention = in.Retention
	in.Storage.DeepCopyInto(&out.Storage)
	in.Resources.DeepCopyInto(&out.Resources)
	out.Encryption = in.Encryption
}

// DeepCopy is an autogenerated deepcopy function, copying the receiver, creating a new BackupConfigSpec.
func (in *BackupConfigSpec) DeepCopy() *BackupConfigSpec {
	if in == nil {
		return nil
	}
	out := new(BackupConfigSpec)
	in.DeepCopyInto(out)
	return out
}

// DeepCopyInto is an autogenerated deepcopy function, copying the receiver, writing into out. in must be non-nil.
func (in *BackupConfigSpecWithSecrets) DeepCopyInto(out *BackupConfigSpecWithSecrets) {
	*out = *in
	in.BackupConfigSpec.DeepCopyInto(&out.BackupConfigSpec)
	in.Storage.DeepCopyInto(&out.Storage)
	if in.Encryption != nil {
		in, out := &in.Encryption, &out.Encryption
		*out = new(BackupEncryptionConfigWithSecrets)
		**out = **in
	}
}

// DeepCopy is an autogenerated deepcopy function, copying the receiver, creating a new BackupConfigSpecWithSecrets.
func (in *BackupConfigSpecWithSecrets) DeepCopy() *BackupConfigSpecWithSecrets {
	if in == nil {
		return nil
	}
	out := new(BackupConfigSpecWithSecrets)
	in.DeepCopyInto(out)
	return out
}

// DeepCopyInto is an autogenerated deepcopy function, copying the receiver, writing into out. in must be non-nil.
func (in *BackupConfigStatus) DeepCopyInto(out *BackupConfigStatus) {
	*out = *in
	if in.FirstRecoverabilityPoint != nil {
		in, out := &in.FirstRecoverabilityPoint, &out.FirstRecoverabilityPoint
		*out = (*in).DeepCopy()
	}
	if in.LastSuccessfulBackup != nil {
		in, out := &in.LastSuccessfulBackup, &out.LastSuccessfulBackup
		*out = (*in).DeepCopy()
	}
	if in.LastFailedBackup != nil {
		in, out := &in.LastFailedBackup, &out.LastFailedBackup
		*out = (*in).DeepCopy()
	}
	if in.ConsumedStorage != nil {
		in, out := &in.ConsumedStorage, &out.ConsumedStorage
		*out = new(ConsumedStorageInfo)
		(*in).DeepCopyInto(*out)
	}
	if in.Conditions != nil {
		in, out := &in.Conditions, &out.Conditions
		*out = make([]metav1.Condition, len(*in))
		for i := range *in {
			(*in)[i].DeepCopyInto(&(*out)[i])
		}
	}
}

// DeepCopy is an autogenerated deepcopy function, copying the receiver, creating a new BackupConfigStatus.
func (in *BackupConfigStatus) DeepCopy() *BackupConfigStatus {
	if in == nil {
		return nil
	}
	out := new(BackupConfigStatus)
	in.DeepCopyInto(out)
	return out
}

// DeepCopyInto is an autogenerated deepcopy function, copying the receiver, writing into out. in must be non-nil.
func (in *BackupConfigWithSecrets) DeepCopyInto(out *BackupConfigWithSecrets) {
	*out = *in
	in.BackupConfig.DeepCopyInto(&out.BackupConfig)
	in.Spec.DeepCopyInto(&out.Spec)
}

// DeepCopy is an autogenerated deepcopy function, copying the receiver, creating a new BackupConfigWithSecrets.
func (in *BackupConfigWithSecrets) DeepCopy() *BackupConfigWithSecrets {
	if in == nil {
		return nil
	}
	out := new(BackupConfigWithSecrets)
	in.DeepCopyInto(out)
	return out
}

// DeepCopyInto is an autogenerated deepcopy function, copying the receiver, writing into out. in must be non-nil.
func (in *BackupEncryptionConfig) DeepCopyInto(out *BackupEncryptionConfig) {
	*out = *in
}

// DeepCopy is an autogenerated deepcopy function, copying the receiver, creating a new BackupEncryptionConfig.
func (in *BackupEncryptionConfig) DeepCopy() *BackupEncryptionConfig {
	if in == nil {
		return nil
	}
	out := new(BackupEncryptionConfig)
	in.DeepCopyInto(out)
	return out
}

// DeepCopyInto is an autogenerated deepcopy function, copying the receiver, writing into out. in must be non-nil.
func (in *BackupEncryptionConfigWithSecrets) DeepCopyInto(out *BackupEncryptionConfigWithSecrets) {
	*out = *in
	out.BackupEncryptionConfig = in.BackupEncryptionConfig
}

// DeepCopy is an autogenerated deepcopy function, copying the receiver, creating a new BackupEncryptionConfigWithSecrets.
func (in *BackupEncryptionConfigWithSecrets) DeepCopy() *BackupEncryptionConfigWithSecrets {
	if in == nil {
		return nil
	}
	out := new(BackupEncryptionConfigWithSecrets)
	in.DeepCopyInto(out)
	return out
}

// DeepCopyInto is an autogenerated deepcopy function, copying the receiver, writing into out. in must be non-nil.
func (in *BackupRetentionConfig) DeepCopyInto(out *BackupRetentionConfig) {
	*out = *in
}

// DeepCopy is an autogenerated deepcopy function, copying the receiver, creating a new BackupRetentionConfig.
func (in *BackupRetentionConfig) DeepCopy() *BackupRetentionConfig {
	if in == nil {
		return nil
	}
	out := new(BackupRetentionConfig)
	in.DeepCopyInto(out)
	return out
}

// DeepCopyInto is an autogenerated deepcopy function, copying the receiver, writing into out. in must be non-nil.
func (in *ConsumedStorageInfo) DeepCopyInto(out *ConsumedStorageInfo) {
	*out = *in
	if in.TotalBytes != nil {
		in, out := &in.TotalBytes, &out.TotalBytes
		*out = new(int64)
		**out = **in
	}
	if in.BackupsBytes != nil {
		in, out := &in.BackupsBytes, &out.BackupsBytes
		*out = new(int64)
		**out = **in
	}
	if in.WALBytes != nil {
		in, out := &in.WALBytes, &out.WALBytes
		*out = new(int64)
		**out = **in
	}
}

// DeepCopy is an autogenerated deepcopy function, copying the receiver, creating a new ConsumedStorageInfo.
func (in *ConsumedStorageInfo) DeepCopy() *ConsumedStorageInfo {
	if in == nil {
		return nil
	}
	out := new(ConsumedStorageInfo)
	in.DeepCopyInto(out)
	return out
}

// DeepCopyInto is an autogenerated deepcopy function, copying the receiver, writing into out. in must be non-nil.
func (in *CustomCAReference) DeepCopyInto(out *CustomCAReference) {
	*out = *in
}

// DeepCopy is an autogenerated deepcopy function, copying the receiver, creating a new CustomCAReference.
func (in *CustomCAReference) DeepCopy() *CustomCAReference {
	if in == nil {
		return nil
	}
	out := new(CustomCAReference)
	in.DeepCopyInto(out)
	return out
}

// DeepCopyInto is an autogenerated deepcopy function, copying the receiver, writing into out. in must be non-nil.
func (in *GCSStorageConfig) DeepCopyInto(out *GCSStorageConfig) {
	*out = *in
	if in.PrefixFrom != nil {
		in, out := &in.PrefixFrom, &out.PrefixFrom
		*out = new(ValueFromSource)
		(*in).DeepCopyInto(*out)
	}
	if in.CredentialsSecretRef != nil {
		in, out := &in.CredentialsSecretRef, &out.CredentialsSecretRef
		*out = new(v1.SecretKeySelector)
		(*in).DeepCopyInto(*out)
	}
}

// DeepCopy is an autogenerated deepcopy function, copying the receiver, creating a new GCSStorageConfig.
func (in *GCSStorageConfig) DeepCopy() *GCSStorageConfig {
	if in == nil {
		return nil
	}
	out := new(GCSStorageConfig)
	in.DeepCopyInto(out)
	return out
}

// DeepCopyInto is an autogenerated deepcopy function, copying the receiver, writing into out. in must be non-nil.
func (in *GCSStorageConfigWithSecrets) DeepCopyInto(out *GCSStorageConfigWithSecrets) {
	*out = *in
	in.GCSStorageConfig.DeepCopyInto(&out.GCSStorageConfig)
}

// DeepCopy is an autogenerated deepcopy function, copying the receiver, creating a new GCSStorageConfigWithSecrets.
func (in *GCSStorageConfigWithSecrets) DeepCopy() *GCSStorageConfigWithSecrets {
	if in == nil {
		return nil
	}
	out := new(GCSStorageConfigWithSecrets)
	in.DeepCopyInto(out)
	return out
}

// DeepCopyInto is an autogenerated deepcopy function, copying the receiver, writing into out. in must be non-nil.
func (in *S3StorageConfig) DeepCopyInto(out *S3StorageConfig) {
	*out = *in
	if in.PrefixFrom != nil {
		in, out := &in.PrefixFrom, &out.PrefixFrom
		*out = new(ValueFromSource)
		(*in).DeepCopyInto(*out)
	}
	if in.RegionFrom != nil {
		in, out := &in.RegionFrom, &out.RegionFrom
		*out = new(ValueFromSource)
		(*in).DeepCopyInto(*out)
	}
	if in.EndpointURLFrom != nil {
		in, out := &in.EndpointURLFrom, &out.EndpointURLFrom
		*out = new(ValueFromSource)
		(*in).DeepCopyInto(*out)
	}
	if in.CustomCA != nil {
		in, out := &in.CustomCA, &out.CustomCA
		*out = new(CustomCAReference)
		**out = **in
	}
	if in.AccessKeyIDRef != nil {
		in, out := &in.AccessKeyIDRef, &out.AccessKeyIDRef
		*out = new(v1.SecretKeySelector)
		(*in).DeepCopyInto(*out)
	}
	if in.AccessKeySecretRef != nil {
		in, out := &in.AccessKeySecretRef, &out.AccessKeySecretRef
		*out = new(v1.SecretKeySelector)
		(*in).DeepCopyInto(*out)
	}
}

// DeepCopy is an autogenerated deepcopy function, copying the receiver, creating a new S3StorageConfig.
func (in *S3StorageConfig) DeepCopy() *S3StorageConfig {
	if in == nil {
		return nil
	}
	out := new(S3StorageConfig)
	in.DeepCopyInto(out)
	return out
}

// DeepCopyInto is an autogenerated deepcopy function, copying the receiver, writing into out. in must be non-nil.
func (in *S3StorageConfigWithSecrets) DeepCopyInto(out *S3StorageConfigWithSecrets) {
	*out = *in
	in.S3StorageConfig.DeepCopyInto(&out.S3StorageConfig)
}

// DeepCopy is an autogenerated deepcopy function, copying the receiver, creating a new S3StorageConfigWithSecrets.
func (in *S3StorageConfigWithSecrets) DeepCopy() *S3StorageConfigWithSecrets {
	if in == nil {
		return nil
	}
	out := new(S3StorageConfigWithSecrets)
	in.DeepCopyInto(out)
	return out
}

// DeepCopyInto is an autogenerated deepcopy function, copying the receiver, writing into out. in must be non-nil.
func (in *StorageConfig) DeepCopyInto(out *StorageConfig) {
	*out = *in
	if in.S3 != nil {
		in, out := &in.S3, &out.S3
		*out = new(S3StorageConfig)
		(*in).DeepCopyInto(*out)
	}
	if in.GCS != nil {
		in, out := &in.GCS, &out.GCS
		*out = new(GCSStorageConfig)
		(*in).DeepCopyInto(*out)
	}
}

// DeepCopy is an autogenerated deepcopy function, copying the receiver, creating a new StorageConfig.
func (in *StorageConfig) DeepCopy() *StorageConfig {
	if in == nil {
		return nil
	}
	out := new(StorageConfig)
	in.DeepCopyInto(out)
	return out
}

// DeepCopyInto is an autogenerated deepcopy function, copying the receiver, writing into out. in must be non-nil.
func (in *StorageConfigWithSecrets) DeepCopyInto(out *StorageConfigWithSecrets) {
	*out = *in
	in.StorageConfig.DeepCopyInto(&out.StorageConfig)
	if in.S3 != nil {
		in, out := &in.S3, &out.S3
		*out = new(S3StorageConfigWithSecrets)
		(*in).DeepCopyInto(*out)
	}
	if in.GCS != nil {
		in, out := &in.GCS, &out.GCS
		*out = new(GCSStorageConfigWithSecrets)
		(*in).DeepCopyInto(*out)
	}
}

// DeepCopy is an autogenerated deepcopy function, copying the receiver, creating a new StorageConfigWithSecrets.
func (in *StorageConfigWithSecrets) DeepCopy() *StorageConfigWithSecrets {
	if in == nil {
		return nil
	}
	out := new(StorageConfigWithSecrets)
	in.DeepCopyInto(out)
	return out
}

// DeepCopyInto is an autogenerated deepcopy function, copying the receiver, writing into out. in must be non-nil.
func (in *ValueFromSource) DeepCopyInto(out *ValueFromSource) {
	*out = *in
	if in.SecretKeyRef != nil {
		in, out := &in.SecretKeyRef, &out.SecretKeyRef
		*out = new(v1.SecretKeySelector)
		(*in).DeepCopyInto(*out)
	}
	if in.ConfigMapKeyRef != nil {
		in, out := &in.ConfigMapKeyRef, &out.ConfigMapKeyRef
		*out = new(v1.ConfigMapKeySelector)
		(*in).DeepCopyInto(*out)
	}
}

// DeepCopy is an autogenerated deepcopy function, copying the receiver, creating a new ValueFromSource.
func (in *ValueFromSource) DeepCopy() *ValueFromSource {
	if in == nil {
		return nil
	}
	out := new(ValueFromSource)
	in.DeepCopyInto(out)
	return out
}

// DeepCopyInto is an autogenerated deepcopy function, copying the receiver, writing into out. in must be non-nil.
func (in *WalgConfig) DeepCopyInto(out *WalgConfig) {
	*out = *in
	if in.GoMaxProcs != nil {
		in, out := &in.GoMaxProcs, &out.GoMaxProcs
		*out = new(int)
		**out = **in
	}
	if in.TotalBgUploadedLimit != nil {
		in, out := &in.TotalBgUploadedLimit, &out.TotalBgUploadedLimit
		*out = new(int)
		**out = **in
	}
	if in.DeltaMaxSteps != nil {
		in, out := &in.DeltaMaxSteps, &out.DeltaMaxSteps
		*out = new(int)
		**out = **in
	}
	if in.DiskRateLimitBytesPerSecond != nil {
		in, out := &in.DiskRateLimitBytesPerSecond, &out.DiskRateLimitBytesPerSecond
		*out = new(int)
		**out = **in
	}
	if in.DownloadConcurrency != nil {
		in, out := &in.DownloadConcurrency, &out.DownloadConcurrency
		*out = new(int)
		**out = **in
	}
	if in.DownloadFileRetries != nil {
		in, out := &in.DownloadFileRetries, &out.DownloadFileRetries
		*out = new(int)
		**out = **in
	}
	if in.FailoverStoragesCheck != nil {
		in, out := &in.FailoverStoragesCheck, &out.FailoverStoragesCheck
		*out = new(bool)
		**out = **in
	}
	if in.NetworkRateLimitBytesPerSecond != nil {
		in, out := &in.NetworkRateLimitBytesPerSecond, &out.NetworkRateLimitBytesPerSecond
		*out = new(int)
		**out = **in
	}
	if in.TarDisableFsync != nil {
		in, out := &in.TarDisableFsync, &out.TarDisableFsync
		*out = new(bool)
		**out = **in
	}
	if in.TarSizeThreshold != nil {
		in, out := &in.TarSizeThreshold, &out.TarSizeThreshold
		*out = new(int64)
		**out = **in
	}
	if in.UploadConcurrency != nil {
		in, out := &in.UploadConcurrency, &out.UploadConcurrency
		*out = new(int)
		**out = **in
	}
	if in.UploadDiskConcurrency != nil {
		in, out := &in.UploadDiskConcurrency, &out.UploadDiskConcurrency
		*out = new(int)
		**out = **in
	}
	if in.PreventWalOverwrite != nil {
		in, out := &in.PreventWalOverwrite, &out.PreventWalOverwrite
		*out = new(bool)
		**out = **in
	}
}

// DeepCopy is an autogenerated deepcopy function, copying the receiver, creating a new WalgConfig.
func (in *WalgConfig) DeepCopy() *WalgConfig {
	if in == nil {
		return nil
	}
	out := new(WalgConfig)
	in.DeepCopyInto(out)
	return out
}
